...
class GraphvizExporter:
    ...
    def export(
        ...
        for theme in themes:
            ...
            tooltip = (
                self._escape(theme.get("description", "")[:100])
            ...
            lines.append(
                f'  {node_id} [{style_str} label="{label}" tooltip="{tooltip}"];'
            )
        ...
        for concept in concepts:
            if concept.get("theme_id"):
                ...
                lines.append(
                    f"  {source} -> {target} [color={style['color']} style={style['style']}];"
                )
    ...
    def _format_style(self, style: dict) -> str:
        ...
        return ", ".join(
            f'{k}="{v}"' if isinstance(v, str) else f"{k}={v}" for k, v in style.items()
        )

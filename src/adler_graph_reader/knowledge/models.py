...
class ConceptNode(BaseModel):
    ...
    arguments: list[Argument] = Field(
        default_factory=list, description="Supporting arguments"
    )
    related_concepts: list[str] = Field(
        default_factory=list, description="Related concept names for linking"
    )
...
class ChapterSummary(BaseModel):
    ...
    key_concepts: list[str] = Field(
        default_factory=list, description="Key concepts mentioned"
    )
...
class BookAnalysis(BaseModel):
    ...
    chapters: list[ChapterSummary] = Field(
        default_factory=list, description="Chapter summaries"
    )

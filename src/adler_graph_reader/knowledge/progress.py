...
class ExtractionProgress:
    ...
    concept_queue: list[str] = field(default_factory=list)  # Concepts waiting to be processed
    processed_concepts: list[str] = field(default_factory=list)  # Concepts already processed
    ...
    def progress_percent(self) -> float:
        ...
        if self.stage == ExtractionStage.THEMES_EXTRACTING and self.total_themes > 0:
            stage_progress += (self.extracted_themes / self.total_themes) * weights[ExtractionStage.THEMES_COMPLETE]
        elif self.stage == ExtractionStage.CONCEPTS_EXTRACTING and self.total_concepts > 0:
            stage_progress += (self.extracted_concepts / self.total_concepts) * weights[ExtractionStage.CONCEPTS_COMPLETE]
        elif self.stage == ExtractionStage.RELATIONS_EXTRACTING and self.total_relations > 0:
            stage_progress += (self.extracted_relations / self.total_relations) * weights[ExtractionStage.RELATIONS_COMPLETE]
...
class ProgressManager:
    ...
    def load_progress(self, document_id: str) -> Optional[ExtractionProgress]:
        ...
        cursor = self.conn.execute(
            "SELECT * FROM extraction_progress WHERE document_id = ?",
            (document_id,)
        )
        row = cursor.fetchone()
        if row:
    ...
    def create_progress(self, document_id: str, config: Optional[dict] = None) -> ExtractionProgress:
    ...
    def delete_progress(self, document_id: str) -> None:
        ...
        self.conn.execute(
            "DELETE FROM extraction_progress WHERE document_id = ?",
            (document_id,)
        )
        self.conn.commit()
    ...
    def get_stalled_tasks(self, threshold_seconds: int = 3600) -> list[ExtractionProgress]:
        ...
        cursor = self.conn.execute(
            ...
            (threshold, ExtractionStage.COMPLETE.value, ExtractionStage.NOT_STARTED.value)
        )
        columns = [desc[0] for desc in cursor.description]
        results = []
        for row in cursor.fetchall():
...
def format_progress_report(progress: ExtractionProgress) -> str:
    ...
    if progress.stage.value >= ExtractionStage.THEMES_EXTRACTING.value:
        lines.append(f"📌 主题提取：{progress.extracted_themes}/{progress.total_themes}")
    
    if progress.stage.value >= ExtractionStage.CONCEPTS_EXTRACTING.value:
        lines.append(f"💡 概念提取：{progress.extracted_concepts}/{progress.total_concepts}")
    ...
    if progress.stage.value >= ExtractionStage.RELATIONS_EXTRACTING.value:
        lines.append(f"🔗 关系提取：{progress.extracted_relations}/{progress.total_relations}")
